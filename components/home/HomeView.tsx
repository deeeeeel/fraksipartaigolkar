...
import { 
  Search, MapPin, Users, Newspaper, Zap, 
  Megaphone, Star, Clock, ChevronRight, Activity, Calendar
} from 'lucide-react';
...
import IndonesiaMap from '../shared/IndonesiaMap';

export const HomeView = ({ initialMembers = [], komisiList = [] }: { initialMembers?: any[], komisiList?: string[] }) => {
  ...
  const membersToUse = initialMembers.length > 0 ? initialMembers : MOCK_MEMBERS;
  const komisiListToUse = komisiList.length > 0 ? komisiList : KOMISI_LIST;
  ...
  const filteredMembers = useMemo(() => {
    return membersToUse.filter((m: any) => {
  ...
  }, [search, activeKomisi, membersToUse]);

  const komisiDistribution = useMemo(() => {
    const counts: Record<string, number> = {};
    membersToUse.forEach((m: any) => {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-10 mb-12">
        ...
        {activeTab === 'peta' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="bg-white p-2 md:p-6 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
               <IndonesiaMap members={membersToUse} />
             </div>
          </div>
        )}
        ...
        {activeTab === 'direktori' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* KOMISI FILTER DENGAN FLEX-WRAP */}
            <div className="flex flex-col gap-4 mb-8 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
              <div className="relative w-full">
                ...
                <input type="text" placeholder="Cari nama anggota atau dapil..." className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-yellow-400 border border-slate-100" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
              <div className="flex flex-wrap items-center gap-2">
                {['Semua', ...komisiListToUse].map((k: string) => (
                  <button key={k} onClick={() => setActiveKomisi(k)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${activeKomisi === k ? 'bg-yellow-400 text-slate-900 border-yellow-400 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50 hover:border-slate-300'}`}>{k}</button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredMembers.map((member: any, idx: number) => {
                ...
                const memberSlug = member.slug || memberName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
                
                return (
                <a href={`/anggota/${memberSlug}`} key={idx} className="bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_15px_35px_rgb(0,0,0,0.08)] hover:-translate-y-2 transition-all duration-300 group cursor-pointer block">
                  <div className="h-56 bg-slate-100 relative overflow-hidden flex items-end justify-center">
                     ...
                     <img 
                        ...
                        onError={(e) => { e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(memberName)}&background=e2e8f0&color=475569&size=400`; }}
                     />
